...
class LabelImage {
  constructor(options) {
	...
	getCreatedLabels = (node, pageY, resultIndex) => {
	  const _self = this
	  const resultSelectLabel = document.querySelector('.resultSelectLabel')
	  const selectLabelUL = resultSelectLabel.querySelector('.selectLabel-ul') // 标签选择UL
	  const closeLabel = resultSelectLabel.querySelector('.closeLabelManage')
	  const selectLabelTip = resultSelectLabel.querySelector('.selectLabelTip')
	  // 加载标签数据
	...
	// ----标签管理
	ManageLabels = () => {
	  const labelSearch = document.querySelector('.labelSearch-input') // 标签搜索
	  const labelManage = document.querySelector('.labelManage') // 标签管理父节点
	  const closeLabel = labelManage.querySelector('.closeLabelManage') // 关闭标签管理窗口节点
	  const labelManegeUL = labelManage.querySelector('.labelManage-ul') // 标签管理列表父节点
	  const labelManageInfo = labelManage.querySelector('.labelManage-Info') // 标签列表模块
	  const labelManageCreateInfo = labelManage.querySelector('.labelManage-create') // 标签编辑模块
	  const addLabel = labelManage.querySelector('.addLabel')
	  const addLabelName = labelManage.querySelector('.labelCreate-nameInput')
	  const addLabelColor = labelManage.querySelector('#colorHex')
	  const labelManageCreate = labelManage.querySelector('.labelManage-createButton') // 添加标签节点
	  const closeAdd = labelManage.querySelector('.closeAdd') // 取消添加标签
	  const removeLabel = labelManage.querySelector('.removeLabel') // 删除标签
	  const colorPicker = labelManage.querySelector('#colorPicker') // 选取颜色模块
	  const labelTip = labelManage.querySelector('.labelTip') // 标签提示
	  const input = document.getElementById('colorHex')
	  const labelManageTitle = document.querySelector('.labelManage-Title')
	  let flag = false
	  let flagIndex = 0
	  const labels = !localStorage.getItem('labels') ? [] : JSON.parse(localStorage.getItem('labels'))
	  if (labels.length > 0) {
	  ...
	  labelSearch.onchange = function(e) {
	    const filterLabel = labels.filter(label => {
	...
	ComputerLabelLocation = (rectMask) => {
	  const x = rectMask.width / 2 + rectMask.xMin
	  const y = rectMask.height / 2 + rectMask.yMin
	  return { x, y }
	};
